...
You need a recent `docker` version installed. Docker provides `instructions <https://docs.docker.com/installation/>` on how to do this.
...
Every configuration option can be set via `environment variables <https://github.com/Graylog2/graylog2-server/blob/master/misc/graylog.conf>`.
...
* The `devicemapper` storage driver can produce problems with Graylogs disk journal on some systems.
  In this case please `pick another driver <https://docs.docker.com/engine/userguide/storagedriver/selectadriver>` like `aufs` or `overlay`. 
